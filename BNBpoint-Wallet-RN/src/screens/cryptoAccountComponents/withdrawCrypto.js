...
import {View, Text, Pressable, Dimensions, Image, Linking} from 'react-native';
...
class WithdrawCrypto extends Component {
  ...
  componentDidMount() {
    this.web3 = new Web3(
      NODE_ENV_NETWORK_RCP[this.context.value.networkSelected],
    );
      ...
      this.mount &&
        this.setState({
          ...
          wallet: {
            kind: 'ethereum',
            address: '0x',
            network: NODE_ENV_NETWORK_ID[this.context.value.networkSelected],
          },
          token: {
  ...
  async onSuccess(e) {
    console.log(e);
    if (e.data.substring(0, 3) === 'wc:') {
      this.WalletConnect = true;
      this.setupConnector(e.data);
    } else if (e.data.substring(0, 2) === '0x') {
      this.WalletConnect = false;
      let temp = {
        kind: 'ethereum',
        address: e.data,
    ...
    } else if (e.data.substring(0, 9) === 'ethereum:') {
      this.WalletConnect = false;
      let temp = {
        kind: e.data.split(':')[0],
  ...
  callBackIoT = data => {
    if (JSON.parse(data[1]).token.substring(0, 3) === 'wc:' && this.scanFlag) {
      ...
      let temp = {
        ...
        network: parseInt(
          JSON.parse(data[1]).token.split(':')[1].split('@')[1],
        ),
      };
      this.mount &&
        this.setState({
  ...
  async maxSelected(token) {
    if (token.label === native[this.context.value.networkSelected]) {
      this.mount &&
        this.setState({
  ...
  async transfer() {
    ...
    if (
      parseFloat(
        this.web3.utils.toHex(this.web3.utils.toWei(this.state.value, 'ether')),
    ...
    console.log({
      ...
      balance: this.context.value.ethBalance,
    });
    if (
      parseFloat(
  ...
  async transferToken(tokenAddress) {
    ...
    const gas = await contract.methods
      .transfer(
        ...
        this.web3.utils.toHex(decodedData.params[1].value),
      )
      .estimateGas({from: this.context.value.account});
    const gasPrice = await this.web3.eth.getGasPrice();
    transaction = {...transaction, gas};
    console.log({
      gas: gas * gasPrice,
      balance: this.web3.utils.toWei(this.context.value.ethBalance, 'ether'),
    });
    if (
      gas * gasPrice >
  ...
  setupConnector(token) {
    this.connector = new WalletConnect({
      uri: token,
      clientMeta: {
        description: NODE_ENV_NETWORK_APPNAME,
    ...
    this.connector.on('call_request', async (error, payload) => {
      ...
      if (payload.method === 'eth_sendTransaction') {
        let ethBalance = await this.web3.eth.getBalance(
          this.context.value.account,
        );
        const gasPrice = await this.web3.eth.getGasPrice();
        ...
        console.log(transaction);
        if (
          tokensContracts[this.context.value.networkSelected]
            .map(item => payload.params[0].to === item)
          ...
          let res = await contract.methods
            .balanceOf(this.context.value.account)
            .call();
          const decodedData = this.abiDecoder.decodeMethod(transaction.data);
          ...
          } else if (decodedData.name === 'approve') {
            const gas = await this.web3.eth.estimateGas(transaction);
            transaction = {...transaction, gas};
            console.log(transaction);
            this.mount &&
        ...
        {this.state.mount && this.state.stage === 0 && (
          <View style={{position: 'absolute', top: 18, right: 18}}>
            <IotReciever
              ...
              sub_topics={[
                `/${NODE_ENV_NETWORK_APPNAME}/WalletConnect/${this.context.value.account}`,
              ]}
        ...
        {this.state.stage === 0.5 && (
          <View
            ...
            <View
              ...
              />
              <Text
                style={{fontSize: 36, fontFamily: 'Helvetica', color: 'white'}}>
        ...
        {this.state.stage === 2 && (
          <View
            ...
            <Text
              ...
              Gas:{' '}
              {epsilonRound(
                this.state.data.gas / Math.pow(10, 18),
                6,
              ).toString()}{' '}
              {native[this.context.value.networkSelected]}
            </Text>
