...
import dai from '../assets/FeedAssets/dai.png';
...
import usdt from '../assets/FeedAssets/usdt.png';
...
class Landing extends Component {
  ...
  componentDidMount() {
    ...
    this.props.navigation.addListener('focus', () => {
      ...
      // Tokens
      tokensContracts[this.context.value.networkSelected].forEach(
        (item, index) => {
  ...
  async getUSDTokens() {
    let array = [geckoNative[this.context.value.networkSelected]].concat(
      geckoTokens,
    );
  ...
  async updatePriceFeeds() {
    let USD = await getUSD(['dai','tether'])
    let [
      priceAVAX,
      priceBNB,
      priceBTC,
    ...
    // To be improve
    const geckoNatTok = [
      geckoNative[this.context.value.networkSelected],
    ].concat(geckoTokens);
    const natTok = [native[this.context.value.networkSelected]].concat(tokens);
    let results = await getUSD(geckoNatTok);
    let tokenUSD = {};
    geckoNatTok.forEach(
      (item, index) => (tokenUSD[natTok[index]] = results[item].usd),
    );
    this.mount &&
      this.setState({
        prices: [
          ...
          parseFloat(priceUSDC.toString()) / 100000000,
          priceUSDT,
